...
const verifyDataPrevista = async ({dtPrevista = '', codObjeto, tipoPostal}) => {
	if(dtPrevista !== '') {
		return dtPrevista;//rastroUnico.formataData(dtPrevista);
	} else {
		// if(rastroUnico.ehTipoPostalInternacional(tipoPostal)) {
		// 	return '';
		// } else {
			const res = await fetch(`dataMaxima.php?objeto=${codObjeto}&tipoPostal=${tipoPostal.tipo}`);
			const r = await res.json();
			if (r.erro) {
				return '';
			} else {
				//const dtMax = JSON.parse(r);
				if(r.dataMaxEntrega !== ''){
				   return rastroUnico.formataData(r.dataMaxEntrega.date);
				}else{
					return '';
				}
			}
        ...
        <div id="cabecalho-rastro" class="d-flex justify-content-between">
            <ul class="cabecalho-rastro">                
                <div class="arrow-dashed justify-content-start">
                	<div class="circle">
                		<img class="circle-img" src="../static/rastreamento-internet/imgs/correios-sf.png" width="35px" height="35px">
                	</div>
                </div>
                <div class="cabecalho-content">
                    <p class="text text-content">${objeto.tipoPostal.categoria}</p>
					     ${objeto.dtPrevista !== '' ?
								...
								objeto.atrasado ?`<p class="text text-head noPrint">Para obter mais informações sobre o objeto, clique <a href="${objeto.urlFaleComOsCorreios}" target="_blank">aqui</a> e registre uma manifestação</p>` :''}	
                </div>                                                
            </ul>        
...
const mostrarDetalhes = async (evento)  => {
	...
	if (!divRastrosUnico.innerHTML.trim().length) {
		...
		if (r.erro) {
			...
			divsBotoes.forEach(el => {el.innerHTML += btns});
...
$(document).ready(function() {
	...
	document.getElementById('b-pesquisar').addEventListener('click', ()=>{busca()},false);
	document.getElementById('objeto').addEventListener('input', ()=>{validarcampoObjeto()},false);
	document.getElementById('captcha_refresh_btn').addEventListener('click', ()=>{refreshCaptcha()},false);
